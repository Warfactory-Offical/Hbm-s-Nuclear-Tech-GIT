...
public class TileEntityMachineReactorSmall extends TileEntity implements ITickable, IFluidHandler, ITankPacketAcceptor {
	...
	public void update() {
		if(!world.isRemote) {
			...
			if(rods > 0 && coreHeat > 0 && !isContained()) {
				...
				final float rad = (float) coreHeat / (float) maxCoreHeat * 50F;
	...
	private boolean isContained() {
		final boolean side1 = blocksRad(pos.add(1, 1, 0));
		if(!side1){
			return false;
		}

		final boolean side2 = blocksRad(pos.add(-1, 1, 0));
		if(!side2){
			return false;
		}

		final boolean side3 = blocksRad(pos.add(0, 1, 1));
		if(!side3){
			return false;
		}

		final boolean side4 = blocksRad(pos.add(0, 1, -1));
        return side4;
    }
	...
	private void generateSteam() {
		...
		final double steam = (((double) hullHeat / (double) maxHullHeat) * ((double) tanks[2].getCapacity() / 50D)) * conversionMod;
	...
	private void getInteractions() {
		...
		final TileEntity te1 = world.getTileEntity(pos.add(2, 0, 0));
		final TileEntity te2 = world.getTileEntity(pos.add(-2, 0, 0));
		final TileEntity te3 = world.getTileEntity(pos.add(0, 0, 2));
		final TileEntity te4 = world.getTileEntity(pos.add(0, 0, -2));

		final boolean b1 = blocksRad(pos.add(1, 1, 0));
		final boolean b2 = blocksRad(pos.add(-1, 1, 0));
		final boolean b3 = blocksRad(pos.add(0, 1, 1));
		final boolean b4 = blocksRad(pos.add(0, 1, -1));

		final TileEntityMachineReactorSmall[] reactors = new TileEntityMachineReactorSmall[4];
	...
	private void getInteractionForBlock(final BlockPos pos) {
		...
		if(b == Blocks.LAVA || b == Blocks.FLOWING_LAVA) {
			...
			final int[] pos1 = ((MachineReactor) ModBlocks.machine_reactor).findCore(world, pos.getX(), pos.getY(), pos.getZ());

			if(pos1 != null) {

				final TileEntity tile = world.getTileEntity(new BlockPos(pos1[0], pos1[1], pos1[2]));
	...
	private void decay(final int id) {
		if(id > 11)
			return;
		...
		for(int i = 0; i < decay; i++) {
			final ItemFuelRod rod = ((ItemFuelRod) inventory.getStackInSlot(id).getItem());
	...
	private void onRunOut(final int id) {

		// System.out.println("aaa");

		final Item item = inventory.getStackInSlot(id).getItem();
	...
	private int getNeightbourCount(final int id) {

		final int[] neighbours = this.getNeighbouringSlots(id);
	...
	private boolean hasFuelRod(final int id) {
		if(id > 11)
			return false;
	...
	private int[] getNeighbouringSlots(final int id) {
	...
	public boolean[] getSubmergedDirection() {
		final boolean[] sides = new boolean[4];
