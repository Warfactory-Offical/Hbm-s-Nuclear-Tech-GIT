...
public class SwitchRotaryToggle extends Control {
    ...
    public SwitchRotaryToggle(final String name, final ControlPanel panel) {
        super(name, panel);
    ...
    @Override
    public void render() {
        final boolean isFlipped = getVar("isOn").getBoolean();
        ...
        tes.setColorRGBA_F(1, 1, 1, 1);
        if (isFlipped) {
            final Matrix4f rot_mat = new Matrix4f().rotate((float) Math.toRadians(-90), new Vector3f(0, 1, 0));
            final Matrix4f trans_mat = new Matrix4f().translate(new Vector3f(posX, 0, posY));
