...
public class NodeDropdown extends NodeElement {
	...
	public NodeDropdown(final Node parent, final int idx, final Function<String, ItemList> func, final Supplier<String> nameGetter){
		super(parent, idx);
	...
	public void render(final float mX, final float mY){
		...
		GL11.glTranslated(-x, -y, 0);
		final String s = nameGetter.get();
		font.drawString(s, x+43-font.getStringWidth(s)/2, y+5, 0xFF5F5F5F, false);
		GL11.glPopMatrix();
		GL11.glPushMatrix();
		final float width = font.getCharWidth('^');
		final float height = font.FONT_HEIGHT;
		x = x+27.5F;
		y = y+2.5F;
