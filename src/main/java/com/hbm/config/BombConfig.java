...
public class BombConfig {
	...
	public static void loadFromConfig(final Configuration config) {
		...
		final Property propGadget = config.get(CATEGORY_NUKES, "3.00_gadgetRadius", 150);
		propGadget.setComment("Radius of the Gadget");
		gadgetRadius = propGadget.getInt();
		final Property propBoy = config.get(CATEGORY_NUKES, "3.01_boyRadius", 120);
		propBoy.setComment("Radius of Little Boy");
		boyRadius = propBoy.getInt();
		final Property propMan = config.get(CATEGORY_NUKES, "3.02_manRadius", 175);
		propMan.setComment("Radius of Fat Man");
		manRadius = propMan.getInt();
		final Property propMike = config.get(CATEGORY_NUKES, "3.03_mikeRadius", 250);
		propMike.setComment("Radius of Ivy Mike");
		mikeRadius = propMike.getInt();
		final Property propTsar = config.get(CATEGORY_NUKES, "3.04_tsarRadius", 500);
		propTsar.setComment("Radius of the Tsar Bomba");
		tsarRadius = propTsar.getInt();
		final Property propPrototype = config.get(CATEGORY_NUKES, "3.05_prototypeRadius", 150);
		propPrototype.setComment("Radius of the Prototype");
		prototypeRadius = propPrototype.getInt();
		final Property propFleija = config.get(CATEGORY_NUKES, "3.06_fleijaRadius", 50);
		propFleija.setComment("Radius of F.L.E.I.J.A.");
		fleijaRadius = propFleija.getInt();
		final Property propMissile = config.get(CATEGORY_NUKES, "3.07_missileRadius", 100);
		propMissile.setComment("Radius of the nuclear missile");
		missileRadius = propMissile.getInt();
		final Property propMirv = config.get(CATEGORY_NUKES, "3.08_mirvRadius", 70);
		propMirv.setComment("Radius of a MIRV");
		mirvRadius = propMirv.getInt();
		final Property propFatman = config.get(CATEGORY_NUKES, "3.09_fatmanRadius", 35);
		propFatman.setComment("Radius of the Fatman Launcher");
		fatmanRadius = propFatman.getInt();
		final Property propNuka = config.get(CATEGORY_NUKES, "3.10_nukaRadius", 25);
		propNuka.setComment("Radius of the nuka grenade");
		nukaRadius = propNuka.getInt();
		final Property propASchrab = config.get(CATEGORY_NUKES, "3.11_aSchrabRadius", 20);
		propASchrab.setComment("Radius of dropped anti schrabidium");
		aSchrabRadius = propASchrab.getInt();
		final Property propSolinium = config.get(CATEGORY_NUKES, "3.12_soliniumRadius", 150);
		propSolinium.setComment("Radius of the blue rinse");
		soliniumRadius = propSolinium.getInt();
		final Property propN2 = config.get(CATEGORY_NUKES, "3.13_n2Radius", 200);
		propN2.setComment("Radius of the N2 mine");
		n2Radius = propN2.getInt();

		final Property propRS1 = config.get(CATEGORY_NUKES, "3.14_riggedStarRadius", 50);
		propRS1.setComment("Radius of the Rigged Star Blaster Energy Cell");
		riggedStarRange = propRS1.getInt();
		final Property propRS2 = config.get(CATEGORY_NUKES, "3.15_riggedStarFuse", 1200);
		...
		final Property propLimitExplosionLifespan = config.get(CATEGORY_NUKE, "6.00_limitExplosionLifespan", 0);
		...
		// explosion speed
		final Property propBlastSpeed = config.get(CATEGORY_NUKE, "6.01_blastSpeed", 1024);
		...
		// fallout range
		final Property propFalloutRange = config.get(CATEGORY_NUKE, "6.02_mk5BlastTime", 40);
		...
		// fallout speed
		final Property falloutRangeProp = config.get(CATEGORY_NUKE, "6.03_falloutRange", 100);
		falloutRangeProp.setComment("Radius of fallout area (base radius * value in percent)");
		falloutRange = falloutRangeProp.getInt();
		// fallout speed
		final Property falloutChunkSpeed = config.get(CATEGORY_NUKE, "6.04_falloutChunkSpeed", 5);
		falloutChunkSpeed.setComment("Process a Chunk every nth tick by the fallout rain");
		fChunkSpeed = falloutChunkSpeed.getInt();
		// new explosion speed
		final Property falloutMSProp = config.get(CATEGORY_NUKE, "6.04_falloutTime", 30);
		...
		final Property spawnFireP = config.get(CATEGORY_NUKE, "6.05_falloutFireSpawn", false);
		...
		final Property disableNuclearP = config.get(CATEGORY_NUKE, "6.06_disableNuclear", false);
		...
		final Property enableNukeNBTSavingP = config.get(CATEGORY_NUKE, "6.08_enableNukeNBTSaving", true);
