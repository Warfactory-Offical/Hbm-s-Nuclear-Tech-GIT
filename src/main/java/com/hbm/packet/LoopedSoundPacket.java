...
public class LoopedSoundPacket implements IMessage {
	...
	public LoopedSoundPacket(final BlockPos pos)
	{
		this.x = pos.getX();
		this.y = pos.getY();
		this.z = pos.getZ();
	...
	public LoopedSoundPacket(final int xPos, final int yPos, final int zPos){
		x = xPos;
		y = yPos;
		z = zPos;
	}
	...
	public static class Handler implements IMessageHandler<LoopedSoundPacket, IMessage> {
		...
		public IMessage onMessage(final LoopedSoundPacket m, final MessageContext ctx) {
			...
			/*	if (te != null && te instanceof TileEntityMachineIGenerator) {
				...
				*/
				...
				if (te != null && te instanceof TileEntityBroadcaster) {
					...
					final int j = te.getPos().getX() + te.getPos().getY() + te.getPos().getZ();
					final int rand = Math.abs(j) % 3 + 1;
