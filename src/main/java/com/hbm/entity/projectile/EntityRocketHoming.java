...
public class EntityRocketHoming extends Entity implements IProjectile {
	...
	public EntityRocketHoming(final World worldIn) {
		super(worldIn);
		if(worldIn.isRemote)
	...
	public EntityRocketHoming(final World world, final double x, final double y, final double z)
    {
        this(world);
        this.setPosition(x, y, z);
    }

    public EntityRocketHoming(final World world, final EntityLivingBase shooter, final EntityLivingBase shootingAt, final float velocity, final float inaccuracy)
    {
    	this(world);
    ...
    public EntityRocketHoming(final World world, final EntityLivingBase shooter, final float velocity, final EnumHand hand)
    {
    	this(world);
        this.shootingEntity = shooter;
    ...
    public EntityRocketHoming(final World world, final int x, final int y, final int z, final double mx, final double my, final double mz, final double grav) {
    	this(world);
    	this.posX = x + 0.5F;
    	this.posY = y + 0.5F;
    	this.posZ = z + 0.5F;
        ...
        else
        {
            ...
            if (movingobjectposition != null && CompatibilityConfig.isWarDim(world))
            {
                ...
                else
                {
                    ...
                    final BlockPos newPos = new BlockPos(this.field_145791_d, this.field_145792_e, this.field_145789_f);
                    
                    final IBlockState state1 = world.getBlockState(newPos);
    ...
    private boolean steer() {
    	final List<Entity> all = world.getEntitiesWithinAABBExcludingEntity(this, new AxisAlignedBB(posX - homingRadius, posY - homingRadius, posZ - homingRadius, posX + homingRadius, posY + homingRadius, posZ + homingRadius));
    	final HashMap<Entity, Double> targetable = new HashMap<Entity, Double>();
    	final Vec3d path = new Vec3d(motionX, motionY, motionZ);
    	final double startSpeed = path.length();
    	path.normalize();
    	...
    	for(final Entity e : all) {
    		...
    		final Vec3d rel = new Vec3d(e.posX - posX, e.posY + e.getEyeHeight() - posY, e.posZ - posZ);
    		final double vecProd = rel.x * path.x + rel.y * path.y + rel.z * path.z;
    		final double bot = rel.length() * path.length();
    		final double angle = Math.acos(vecProd / bot) * 180 / Math.PI;
    	...
    	for(final Map.Entry<Entity, Double> entry : targetable.entrySet()) {
    	...
    	final Vec3d winVec = new Vec3d(nearestE.posX - posX, nearestE.posY - posY, nearestE.posZ - posZ);
    	
    	winVec.normalize();
    	
    	final double newX = ((path.x * (smallest * homingMod - 1)) + winVec.x) / (smallest * homingMod);
    	final double newY = ((path.y * (smallest * homingMod - 1)) + winVec.y) / (smallest * homingMod);
    	final double newZ = ((path.z * (smallest * homingMod - 1)) + winVec.z) / (smallest * homingMod);
