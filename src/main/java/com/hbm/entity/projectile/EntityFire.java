...
public class EntityFire extends Entity implements IProjectile {
	...
	public EntityFire(final World worldIn) {
		super(worldIn);
		if(worldIn.isRemote)
	...
	public EntityFire(final World w, final double x, final double y, final double z){
		this(w);
		this.setPosition(x, y, z);
	}
	
	public EntityFire(final World w, final EntityLivingBase shooter, final EntityLivingBase shootingAt, final float velocity, final float inaccuracy){
		this(w);
	...
	public EntityFire(final World w, final EntityLivingBase shooter, final float velocity, final EnumHand hand){
		this(w);
		this.shootingEntity = shooter;
	...
	public EntityFire(final World world, final int x, final int y, final int z, final double mx, final double my, final double mz, final double grav) {
        this(world);
    	this.posX = x + 0.5F;
    	this.posY = y + 0.5F;
    	this.posZ = z + 0.5F;
        ...
        else
        {
            ...
            if (movingobjectposition != null && CompatibilityConfig.isWarDim(world))
            {
                ...
                else
                {
                    ...
                    final IBlockState st = this.world.getBlockState(newPos);
