...
public class RenderTurretMaxwell extends RenderTurretBase<TileEntityTurretMaxwell> {
	...
	@Override
	public boolean isGlobalRenderer(final TileEntityTurretMaxwell te) {
		return te.beam > 0;
	}
	
	@Override
	public void render(final TileEntityTurretMaxwell turret, final double x, final double y, final double z, final float partialTicks, final int destroyStage, final float alpha){
		...
		if(turret.beam > 0) {
			...
			final double length = turret.lastDist - turret.getBarrelLength()+offset+0.2;
			...
			final long worldTime = turret.getWorld().getTotalWorldTime();
			// for(int i = 0; i < 8; i++)
