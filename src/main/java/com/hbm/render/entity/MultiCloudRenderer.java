...
public class MultiCloudRenderer extends Render<EntityModFX> {
	...
	public MultiCloudRenderer(final Item[] items, final RenderManager renderManager) {
		super(renderManager);
		textureItems = items;
		meta = 0;
	}
	public MultiCloudRenderer(final Item[] items, final int m, final RenderManager renderManager) {
		super(renderManager);
		textureItems = items;
		meta = m;
	}
	...
	@Override
	public void doRender(final EntityModFX fx, final double x, final double y, final double z, final float entityYaw, final float partialTicks) {
		...
		if (tex != null) {
			...
			////
			final Random randy = new Random(fx.hashCode());
			////
			...
			for(int i = 0; i < 5; i++) {
				...
				final float d = randy.nextInt(10) * 0.05F;
				...
				final double size = rand.nextDouble() * 0.5D + 0.25D;
	...
	@Override
	protected ResourceLocation getEntityTexture(final EntityModFX fx) {
	...
	private void func_77026_a(final Tessellator tes, final TextureAtlasSprite tas) {
		final BufferBuilder buf = tes.getBuffer();
		final float f = tas.getMinU();
		final float f1 = tas.getMaxU();
		final float f2 = tas.getMinV();
		final float f3 = tas.getMaxV();
		final float f4 = 1.0F;
		final float f5 = 0.5F;
