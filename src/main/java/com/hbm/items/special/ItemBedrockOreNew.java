...
public class ItemBedrockOreNew extends Item {
    ...
    public String getItemStackDisplayName(ItemStack stack) {
        int meta = stack.getItemDamage();
        String type = I18n.format(this.getTranslationKeyInefficiently(stack) + ".type." + this.getType(meta).suffix + ".name");
        return I18n.format(this.getTranslationKeyInefficiently(stack) + ".grade." + this.getGrade(meta).name().toLowerCase(Locale.US) + ".name", type);
    }
    ...
    public void addInformation(ItemStack stack, World worldIn, List<String> list, ITooltipFlag flagIn) {
        ...
        for(ProcessingTrait trait : this.getGrade(stack.getItemDamage()).traits) {
            list.add(I18nUtil.resolveKey(this.getTranslationKeyInefficiently(stack) + ".trait." + trait.name().toLowerCase(Locale.US)));
        }
    }
