...
public class GunImmolator extends Item {
	...
	public GunImmolator(final String s) {
	...
	public void onUsingTick(final ItemStack stack, final EntityLivingBase player1, final int count) {
		...
		if (!player.isSneaking()) {
			...
			if ((player.capabilities.isCreativeMode || Library.hasInventoryItem(player.inventory, ModItems.gun_immolator_ammo))) {
				final EntityFire entityarrow = new EntityFire(world, player, 3.0F, player.getHeldItemMainhand() == stack ? EnumHand.MAIN_HAND : EnumHand.OFF_HAND);
			...
			if ((player.capabilities.isCreativeMode || Library.hasInventoryItem(player.inventory, ModItems.gun_immolator_ammo))) {
				...
				final EntityPlasmaBeam plasma = new EntityPlasmaBeam(world, player, 1F, player.getHeldItemMainhand() == stack ? EnumHand.MAIN_HAND : EnumHand.OFF_HAND);
