...
public class TurretCIWS extends TurretBase {
	...
	public TurretCIWS(final Material materialIn, final String s) {
		super(materialIn, s);
	}
	...
	public boolean executeHoldAction(final World world, final int i, final double yaw, double pitch, final BlockPos pos) {
		...
		final TileEntityTurretCIWS te = (TileEntityTurretCIWS)world.getTileEntity(pos);
		...
		if(te.spin > 25 && i % 2 == 0) {
			final Vec3d vector = new Vec3d(0, 0, 1).rotatePitch((float)Math.toRadians(-pitch)).rotateYaw((float)Math.toRadians(-yaw));
			if(!world.isRemote) {
				
				rayShot(world, vector, x + vector.x * 2.5 + 0.5, y + vector.y * 2.5 + 0.5, z + vector.z * 2.5 + 0.5, 200, 10.0F, WeaponConfig.ciwsHitrate);
				
				final EntityGasFlameFX smoke = new EntityGasFlameFX(world);
	...
	private void rayShot(final World world, final Vec3d vec, final double posX, final double posY, final double posZ, final int range, final float damage, final int hitPercent) {
		...
		final Vec3d test = new Vec3d(vec.x * range + posX, vec.y * range + posY, vec.z * range + posZ);
		final Vec3d pos = new Vec3d(posX, posY, posZ);
		final AxisAlignedBB box = new AxisAlignedBB(pos.x, pos.y, pos.z, test.x, test.y, test.z).grow(1.0D);
		final List<Entity> ents = world.getEntitiesWithinAABBExcludingEntity(null, box);
