...
public class TurretCheapo extends TurretBase {
	...
	public TurretCheapo(final Material materialIn, final String s) {
		super(materialIn, s);
	}
	...
	public boolean executeHoldAction(final World world, final int i, final double yaw, double pitch, final BlockPos pos) {
		...
		final TileEntityTurretCheapo te = (TileEntityTurretCheapo) world.getTileEntity(pos);
		...
		if (te.spin > 25 && i % 2 == 0) {
			final Vec3d vector = new Vec3d(-Math.sin(yaw / 180.0F * (float) Math.PI) * Math.cos(pitch / 180.0F * (float) Math.PI), -Math.sin(pitch / 180.0F * (float) Math.PI), Math.cos(yaw / 180.0F * (float) Math.PI) * Math.cos(pitch / 180.0F * (float) Math.PI));
