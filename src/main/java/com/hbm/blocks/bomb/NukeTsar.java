...
public class NukeTsar extends BlockContainer implements IBomb {
	...
	public NukeTsar(final Material materialIn, final String s) {
		super(materialIn);
	...
	public boolean onBlockActivated(final World world, final BlockPos pos, final IBlockState state, final EntityPlayer player, final EnumHand hand, final EnumFacing facing, final float hitX, final float hitY, final float hitZ) {
		if(world.isRemote) {
			...
			final TileEntityNukeTsar entity = (TileEntityNukeTsar) world.getTileEntity(pos);
			if(entity != null) {
	...
	public void neighborChanged(final IBlockState state, final World worldIn, final BlockPos pos, final Block blockIn, final BlockPos fromPos) {
		final TileEntityNukeTsar entity = (TileEntityNukeTsar) worldIn.getTileEntity(pos);
		if(worldIn.isBlockPowered(pos) && !worldIn.isRemote) {
			final boolean isReady = entity.isReady();
			final boolean isStage1Filled = entity.isStage1Filled();
			final boolean isStage2Filled = entity.isStage2Filled();
			final boolean isStage3Filled = entity.isStage3Filled();
			if(isStage3Filled) {
	...
	@Override
	public void explode(final World worldIn, final BlockPos pos) {
		final TileEntityNukeTsar entity = (TileEntityNukeTsar) worldIn.getTileEntity(pos);
		final boolean isReady = entity.isReady();
		final boolean isStage1Filled = entity.isStage1Filled();
		final boolean isStage2Filled = entity.isStage2Filled();
	...
	public void addInformation(final ItemStack stack, final World player, final List<String> tooltip, final ITooltipFlag advanced) {
		...
		if(!BombConfig.disableNuclear){
			...
			tooltip.add(" §e"+I18nUtil.resolveKey("desc.radius", BombConfig.tsarRadius *(1+BombConfig.falloutRange/100))+"§r");
		}
	}
}
